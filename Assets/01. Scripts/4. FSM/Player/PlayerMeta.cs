...
public class PlayerMeta : MonoBehaviour, IPlayerState<PlayerController>
{
    public void StateEnter(PlayerController sender)
    {
        ...
        if (player != null)
        {
            InGameManager.Instance?.PlaceMetaAtCell(player.nowPos);
        }

        // 작업 끝나면 상태 해제(또는 Idle 상태로 전환)
        sender.sm.SetState(sender.dicState[PlayerState.Idle]);
    }
